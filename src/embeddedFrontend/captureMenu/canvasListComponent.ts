    ...
    export class CanvasListComponent extends BaseComponent<ICanvasListComponentState> {
        ...
        public render(state: ICanvasListComponentState, stateId: number): Element {
            ...
            <div class="canvasListComponent">
                <span commandName="onCanvasSelection">${state.currentCanvasInformation ? `${state.currentCanvasInformation.id} (${state.currentCanvasInformation.width}*${state.currentCanvasInformation.height})` : "Choose Canvas..."}</span>
